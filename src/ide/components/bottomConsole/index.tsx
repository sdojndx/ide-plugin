...
export default function BottomConsole() {
  ...
  const { clearOutputText, setOutputText, originOutputText, outputText } = useIdeStore();
  const { clearEventData } = useIdeStore();
  const { clearWorldState } = useIdeStore();
  ...
  const { currentTab, setCurrentTab, setIsHideBottom } = useIdeStore();
