...
export default forwardRef(function CodeMirrorEditor(
  ...
  const { server, removeEditor, updateEditor, openEditor, updateLints, lints } =
    useIdeStore();
  // 服务器端代码缓存
  ...
  useEffect(() => {
    const state = EditorState.create({
      ...
      extensions: [
        ...
        langConfig.current.of(
          StreamLanguage.define(
            typeMap[editor.fileType as keyof typeof typeMap],
          ),
        ),
  ...
  useEffect(() => {
    ...
    if (editorView.current) {
      editorView.current?.dispatch({
        effects: editorableConfig.current.reconfigure(
          EditorView.editable.of(
            editor.editable === undefined ? true : editor?.editable,
          ),
        ),
      });
    }
  ...
  useEffect(() => {
    ...
    editorView?.current?.dispatch({
      effects: updateConfig.current.reconfigure(
        EditorView.updateListener.of((info) => {
  ...
  const decl = useCallback(
    async (event?: MouseEvent) => {
      ...
      const code = editorView.current.state.doc.toString() || '';
      const pos = editorView.current.posAtCoords({
        x: event.clientX,
        y: event.clientY,
      });
      if (!pos) {
      ...
      case 'cursor':
        editorView.current?.dispatch({
          ...
          selection: EditorSelection.cursor(
            getPostByLineAndCh(code, editor.action.Line, editor.action.Ch),
          ),
          scrollIntoView: true,
        });
  ...
  return <div style={editorStyle} className={className} ref={dom}></div>;
});
