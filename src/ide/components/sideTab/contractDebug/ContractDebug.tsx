...
import addParamIcon from '../../../static/svgs/add-icon.svg';
import deleteParamIcon from '../../../static/svgs/delete-icon.svg';
...
export default function ContractDebug({ style }: {
  ...
  const getContractMethod = useCallback(async () => {
    ...
    const data = await server?.getContractMethod(selectedContract.path);
  ...
  const runContract = useCallback(async () => {
    ...
    server?.postContractInvokeAll(param).then(result => {
      setShowLoading(false);
      // 日志输出
