...
import { index, getUser, logOut } from '@/api';
...
export default function Home() {
  ...
  const [defaultSetting, setDefaultSetting] = useState<Partial<IdeSetting['setting']>>({});
  ...
  const checkContract = useCallback((contractName: string) => {
    ideServer?.getContractFiles?.({ contractName, includeApi: false }).then(res => {
  ...
  return <>
    {contractName && pageActionReady && <Ide ref={(r) => (editor.current = r)} header={
      <div className="zx_header_ide">
        <div className="zx_header_icon">
          <img src={logo} />
        </div>
        <div className="zx_header_flex"></div>
        <a target="_blank" href={import.meta.env.VITE_CONSOLE_HOST} className="zx_header_link" rel="noreferrer">控制台</a>
        <a target="_blank" href="https://zxchain.qq.com/" className="zx_header_link" rel="noreferrer">进入官网</a>
        <a target="_blank" href={`${import.meta.env.VITE_CONSOLE_HOST}docs?docpath=doc/content/%E4%BA%A7%E5%93%81%E7%AE%80%E4%BB%8B/%E4%BA%A7%E5%93%81%E6%A6%82%E8%BF%B0.html`} className="zx_header_link" rel="noreferrer">帮助文档</a>
        <div className="zx_header_user">
          {user?.name}
        </div>
        <div onClick={logout} className="zx_header_link">退出</div>
      </div>
    } ideServer={ideServer} userId={user?.secretId} contractName={contractName} defaultSetting={defaultSetting}></Ide>}
  </>;
}
