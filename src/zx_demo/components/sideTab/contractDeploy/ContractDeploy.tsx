...
import { DeployContractListResponse } from '@ide/types/ideEventListener';
...
export default function ContractDeploy({ style }: {
  ...
  const { contract } = useAppStore();
  const { setOriginOutputText } = useAppStore();
  ...
  const queryDeployContractList = useCallback(async () => {
    const res = await appServer?.queryDeployContractList?.();
  ...
  const deployContract = useCallback(async () => {
    ...
    // 合约部署-获取7z
    const file: File = await appServer?.getContractFile?.(getSelected.path);
      ...
      try {
        ...
        appServer?.pluginRequest({
          ...
          if (res.message === 'OK' && res.txId) {
            // 请求deploy接口
            appServer?.deployContract?.({ contractName: getSelected.contractName, hash: projectNameRelate[projectName].hash });
    ...
    //   description: '部署成功'
    // });
  }, [extraList, projectName, projectNameOptions, projectNameRelate, ideEventListener]);
