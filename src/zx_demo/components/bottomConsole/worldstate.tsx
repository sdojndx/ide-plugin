...
export default function IDEWorldState({ style }: {
  ...
  const { contract, setAlartModalContent, worldState, unshiftWorldState, setWorldState } = useAppStore();
  ...
  const addData = useCallback((type?: string) => {
    if (selectedContract && selectedContract.contractName) {
      ...
      if (list) {
        ...
        if (count >= 100) {
          if (type !== 'auto') {
            setAlartModalContent('已超过最大上限，无法继续添加');
          }
          return false;
        }
      }
    ...
    if (selectedContract && selectedContract.contractName) {
      if (type === 'key') {
        if (value && tree[selectedContract.contractName][value]) {
          setAlartModalContent('输入的属性不能重复哦！');
          return;
        }
